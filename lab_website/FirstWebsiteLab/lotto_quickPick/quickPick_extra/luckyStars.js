...
function getRandomArbitrary(min, max, lines, perLine, totalLS, lsPerLine) {
	...
	for(var j = 0; j < lines; j++){
		...
		/*check for lucky star requests and add to line if here - can just use same logic again as for adding numbers per line above*/
		if((totalLS)&&(lsPerLine)){
			for(var i = 0; i < lsPerLine; i++){
				var x = Math.ceil(Math.random() * (totalLS - min) + min);
				if(i == 0){
					line.push(x);	
				}else{
					if(line.indexOf(x)>-1){ //check if number already selected.
